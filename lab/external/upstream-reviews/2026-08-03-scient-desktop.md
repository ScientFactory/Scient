...
| Synara | `9265ca5a` | Makes a just-sent message visibly glide to its transcript anchor without reversing, teleporting, or drifting after landing. | 5/5; uses one clock, seeds motion from the measured visible offset, replaces an asymptotic approach with a bounded cubic ease-out, and strengthens browser proof against teleporting. | Scient smooths only the first post-send bottom re-stick in `ChatView`, relies on `MessagesTimeline` for list-tail following, and proves a smooth-scroll request plus bottom distance; it has no Synara anchor hook or visible multi-frame glide proof. | High concept | Low direct | Hard | Scroll geometry, motion, virtualization, layout mutation, reduced motion, user takeover, browser timing, absent owned anchor lifecycle, and human appearance judgment | **Defer pending human reproduction; automatically ineligible today.** | No | Reproduce the current motion with a human; reimplement a Scient-owned anchor lifecycle only if that confirms a real gap, with browser geometry, reduced-motion, and manual visual acceptance. |
