...
| `8f0e1eb7` priority and control admission | Orders user, control, and recovery work without bypassing admission rules. | **Defer.** The invariant is useful, but it depends on the donor command/admission state machine and protected concurrency policy. |
| `8f0e1eb7` Claude stale-resume invalidation | Prevents stale Claude resume state from surviving revert or recovery transitions. | **Defer.** Provider/session ownership is actively overlapping and needs a current Scient-specific stale-resume repro. |
...
| `cd092c0d` rescue-ref cleanup and failure capture | Cleans temporary rescue references and makes revert cleanup failures observable without hiding the primary failure. | **Defer.** Git recovery and data-loss authority require a bounded Scient rescue-ref design plus leak/failure regression proof. |
