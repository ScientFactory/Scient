...
| **Workspace Health and Guidance Audit skill.** Design an optional project-scoped skill that starts read-only and reports whether a workspace is understandable and well organized. It should identify stale, missing, duplicated, contradictory, or excessive instructions and useful structural improvements without imposing a generic template or automatically rewriting the project. The exact checks, workspace types, activation default, remediation workflow, and validation cases remain open. | Yaacov | 2026-08-07 | Spoken idea raised during the T3-derived migration. Evaluate it against the accepted built-in skill identity and activation boundary before authoring or release. | Skills candidate catalog, project initialization, workspace orientation, agent-guidance stewardship, repository organization, and safe remediation planning |
...
| **OS-aware download experience for the Scient website.** Study the Synara website (`https://www.trysynara.com/`) generally and specifically adopt lessons from its download flow to improve the Scient website. Notable patterns: the page detects the visitor's operating system and highlights the matching platform card as "For your device"; three per-platform cards (macOS/Windows/Linux) each show format and architecture; the macOS card uses an in-card "Apple Silicon / Intel" segmented switch to pick the build rather than listing separate rows; a download-count line provides social proof; and a "Browse all releases" link routes power users to older versions and checksums. The hero also uses an OS-adaptive "Download for macOS" primary button paired with "Star on GitHub" and a provider-icon trust row. Adapt these to Scient's own design, platform/build matrix, accessibility, and brand — inspiration, not a design to copy. Exact detection behavior, fallback when detection is uncertain, and which patterns to adopt remain to be decided; compare against the current Scient website download experience before implementing. | Yaacov | 2026-07-25 | Spoken website idea prompted by admiring the Synara download flow (OS detection + Apple Silicon/Intel switch). Privacy-clear visual references are stored as **VR-020** (download page) and **VR-021** (landing hero) in [marketing-and-website visual references](../research/visual-references/marketing-and-website/README.md#vr-020--synara-download-page). Related to the existing "Website FAQ and provider-logo presentation" idea, which also names the Synara website as a reference. | Website product design, download/distribution UX, OS and architecture detection, build/release matrix, social proof, accessibility, brand review, and implementation planning |
...
| **Let agents see the in-app browser, provider-independently.** Work on making agents able to perceive what is shown in Scient's in-app browser — the content and state of the pages the researcher has open — and implement that capability. Design it to be as little "provider-dependent" as possible so it works with any provider rather than only one, if achievable. The exact perception mechanism (semantic snapshot, screenshot/vision, DOM/accessibility tree, or a combination), how it is exposed to different providers with differing capabilities, and the fallback when a provider cannot support the full experience all remain to be evaluated. | Yaacov | 2026-07-26 | Spoken idea. Narrows and sharpens the sibling "Provider-neutral collaborative browser automation" entry above by emphasizing agent *perception* of the in-app browser as the priority and making cross-provider support an explicit design goal. Evaluate together with that entry. | Browser product planning, agent perception and tool/capability contracts, cross-provider abstraction, desktop architecture, permissions and privacy, evidence capture, and cross-provider quality evaluation |
...
| Idea | Raised by | Date added | Context or source | Possible area |
|---|---|---|---|---|
| **Claude-style composer attachment context redesign.** Redesign the selected attachment or context surface above the composer—for example, the current compact card showing “Batch 1 — 50 rows selected live …” with a “Show in text field” action—so it feels closer to the way the Claude app used to present attached context. Keep the selected material identifiable, compact, removable, and inspectable while making the distinction between attached context and text inserted into the prompt clear. The exact Claude reference, first supported attachment states, information hierarchy, and interaction model still need design review; the reference is inspiration, not an approved design to copy. | Yaacov | 2026-07-25 | Spoken product idea with supplied screenshot of the current Scient composer attachment/context surface. The requested reference is the former Claude app treatment. | Composer UX, attachment and selection context, context chips, batch/table selections, accessibility, and responsive desktop/mobile design |
| **Quick fix: excessive gap between last chat message and the composer while a task is running.** The vertical space between the bottom of the last message in the chat area and the composer is too large, specifically when a task is running and the running-task panel (e.g. "2 out of 5 tasks completed") is shown above the composer. Tighten the spacing so the conversation does not feel disconnected from the composer during an active run. | Yaacov | 2026-07-26 | Spoken quick-fix idea with supplied screenshot of a thread with a task running, showing the oversized gap above the running-task/composer area. | Desktop chat layout, composer spacing, running-task panel, and CSS/layout quick fix |
...
| **Route notifications deliberately instead of moving all of them into Activity.** Keep an Activity area as a high-priority post-migration feature, but decide the owner of each notification type individually: Activity, its contextual surface, the inherited notification system, or a deliberate combination without duplicate attention signals. The first candidate is a connected provider CLI update: retain it in Activity and show a contextual indicator beside the composer model picker. | Yaacov | 2026-08-07 | Spoken migration-period idea. Reconcile with the legacy Activity design, current inherited notification behavior, and the provider-lifecycle plan before implementation. | Notification ownership, sidebar Activity, provider lifecycle, composer/model-picker UX, read state, accessibility, and mobile behavior |
...
| Idea | Raised by | Date added | Context or source | Possible area |
|---|---|---|---|---|
| **In-app learning academy.** Add a Scient in-app "academy" that teaches users how to use the product through structured, hands-on courses, adapting the idea from Factory AI's "Factory Academy". The referenced design presents a full-page learning hub (reached from the account menu) with a left module-navigation sidebar and a card-grid catalog: courses are grouped into named tiers, each course card shows lesson count, estimated duration, and a difficulty pill, later modules stay visibly locked with an "Unlocks after passing …" prerequisite line, and completing a tier can earn a certification. Adapt the concept and the admired visual treatment to Scient's own scientific workflows, curriculum, brand, and accessibility needs — the reference is inspiration, not an approved design to copy, and Factory's product-specific curriculum, gamified quiz/certification mechanics, and exact layout remain open questions. | Yaacov | 2026-07-25 | Spoken idea explicitly modeled on Factory AI's "Factory Academy", with a supplied screenshot Yaacov called beautifully designed. A privacy-redacted visual reference is stored as **VR-019** in [dashboard-and-settings visual references](../research/visual-references/dashboard-and-settings/README.md#vr-019--factory-ai-factory-academy-learning-hub). | Product planning, onboarding and education UX, in-app learning surface, curriculum and content architecture, progression/certification model, design, and accessibility |
