...
Everything here is raw research evidence. A saved reference is not an accepted LitRev interaction pattern, severity model, accessibility claim, implementation requirement, or instruction to copy another product.
...
- **Why it was saved:** Yaacov wants this available as a reference for an important or blocking popup pattern during later LitRev design work.
...
- **Status:** Raw visual reference only. It is saved for comparison and inspiration, not as an accepted LitRev modal, severity pattern, interaction rule, visual design, or implementation commitment.
